...
class Sample():
	def __init__(self,sample_name,bam_file,vcf_file,call_file,output_file,gt='hg19',gf='/home/xwu/reference/hs37d5/hs37d5.fa',population='CEU'):
		...
		self._ref={}
		print 'SL '
		self._ref['isize'],self._ref['length']=self._bam.SL(self._sv_ref)#ci & length
		#print 'rpkm1'
		#self._ref['rpkm']=self._bam.rpkm(self._sv_ref)# rpkm
		#print 'coverage1'
		#self._ref['coverage']=self._bam.coverage(self._sv_ref)# coverage
		print 'snvcov1'
		self._ref['snvcov']=self._snv.snv(self._sv_ref)# snv_cov nsnv ratio nhet

		print 'sample sv'
		self._sv_sample=SV(call_file,sample_name,gt,gf,sample_flag=True)
		self._sample={}
		print 'rpkm2'
		self._sample['rpkm']=self._bam.rpkm(self._sv_sample)
		print 'discordant'
		self._sample['discordant']=self._bam.discordant(self._sv_sample,median(self._ref['isize'].values()))
		print 'snvcov2'
		self._sample['snvcov']=self._snv.snv(self._sv_sample)
		print 'tagcnv'
		self._sample['tagcnv']=self.tagCnvPercent()
		
		print 'write'
		self.write_features(output_file)
	...
	def write_features(self,output_file):
